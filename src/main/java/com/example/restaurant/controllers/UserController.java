...
public class UserController {
  ...
  @Operation(
      ...
      description =
          "Initiates the email update process by saving the new email "
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Verification link sent to the new email address"),
        @ApiResponse(
            responseCode = "400",
            description = "Email is invalid or already in use by another user"),
        @ApiResponse(
            responseCode = "401",
            description = "Unauthorized - Valid JWT token is required"),
        @ApiResponse(responseCode = "404", description = "User not found"),
  ...
  public ResponseEntity<ResultHandler<Void>> updateEmail(
      @RequestParam @Parameter(description = "New email") @Email @NotBlank String email,
      @AuthenticationPrincipal(expression = "token") String userToken) {

    _userServices.updateEmail(userToken, email);
    return ResponseEntity.ok(
        ResultHandler.success("Verification link sent to the new email", HttpStatus.OK.value()));
  }

  @Operation(
      summary = "Confirm email change",
      description =
          "Finalizes the email update process by validating "
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "400",
            description = "Invalid/expired token, or no pending email update found"),
        @ApiResponse(
            responseCode = "401",
            description = "Unauthorized - Valid JWT token is required"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
  ...
  public ResponseEntity<ResultHandler<Void>> deleteUser(
      @AuthenticationPrincipal(expression = "token") String userToken) {
  ...
  @Operation(
      ...
      description =
          "Creates a new employee account (Manager or Waiter) and automatically activates it. "
  ...
  public ResponseEntity<ResultHandler<Void>> createEmployee(
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(ResultHandler.success("Employee created successfully", HttpStatus.CREATED.value()));
  }

  @Operation(
      summary = "Edit employee details",
      description =
          "Updates the email and/or username of an existing employee. "
  ...
  public ResponseEntity<ResultHandler<Void>> editEmployee(
    ...
    return ResponseEntity.ok(
        ResultHandler.success("Employee updated successfully", HttpStatus.OK.value()));
  }

  @Operation(
      summary = "Change employee password",
      description =
          "Allows a manager to forcefully change an employee's password. "
  ...
  public ResponseEntity<ResultHandler<Void>> changeEmployeePassword(
    ...
    return ResponseEntity.ok(
        ResultHandler.success("Employee password changed successfully", HttpStatus.OK.value()));
  }

  @Operation(
      summary = "Change employee role",
      description =
          "Allows a manager to change the role of an employee "
  ...
  public ResponseEntity<ResultHandler<Void>> changeEmployeeRole(
    ...
    return ResponseEntity.ok(
        ResultHandler.success("Employee role changed successfully", HttpStatus.OK.value()));
  }

  @Operation(
      summary = "Change employee availability (Block/Unblock)",
      description =
          "Allows a manager to block or unblock an employee's access to the system. "
  ...
  public ResponseEntity<ResultHandler<Void>> blockEmployee(
    ...
    return ResponseEntity.ok(
        ResultHandler.success("Employee availability changed successfully", HttpStatus.OK.value()));
  }

  @Operation(
      summary = "Delete employee",
      description =
          "Performs a soft delete of an employee account, "
  ...
  public ResponseEntity<ResultHandler<Void>> deleteEmployee(
    ...
    return ResponseEntity.ok(
        ResultHandler.success("Employee deleted successfully", HttpStatus.OK.value()));
  }

  @Operation(
      summary = "Generate 2FA QR code for setup",
      description =
          "Generates a new 2FA secret and returns a QR code URI along with a manual code. "
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "403",
            description = "Forbidden - User does not have ROLE_MANAGER role"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  ...
  public ResponseEntity<ResultHandler<Generate2faResponse>> generate2fa(
    ...
    return ResponseEntity.ok(
        ResultHandler.success(
            "2FA secret generated successfully", HttpStatus.OK.value(), response));
  }

  @Operation(
      summary = "Verify code and enable 2FA on account",
      description =
          "Verifies the 6-digit code from the authenticator app. "
