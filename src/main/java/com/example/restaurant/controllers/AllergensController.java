...
public class AllergensController {
  ...
  @Operation(
      ...
      description =
          "Retrieves a dictionary list of all allergens available in the system. "
  ...
  @Operation(
      ...
      description =
          "Creates a new allergen in the system. "
  ...
  public ResponseEntity<ResultHandler<Void>> add(@RequestBody @Valid AddEntityRequest request) {
    ...
    return ResponseEntity.ok(
        ResultHandler.success("Allergen created successful", HttpStatus.CREATED.value()));
  }

  @Operation(
      summary = "Remove a allergen (Soft Delete)",
      description =
          "Marks a dish as allergen by setting its availability to false, "
  ...
  @PreAuthorize("hasAnyRole('ROLE_MANAGER')")
  public ResponseEntity<ResultHandler<Void>> removeAllergen(@PathVariable String token) {
    _allergensServices.remove(token);

    return ResponseEntity.ok(
        ResultHandler.success("Allergen removed successfully", HttpStatus.OK.value()));
  }
}
