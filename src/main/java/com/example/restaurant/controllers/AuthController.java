...
public class AuthController {
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "403",
            description = "Account is disabled or has no permissions"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  @PostMapping("/login")
  public ResponseEntity<ResultHandler<AuthResponse>> login(
      @Valid @RequestBody LoginRequest request) {
    var response = _authServices.authenticate(request);
    return ResponseEntity.ok()
        .headers(createLoginCookieHeaders(response))
        .body(
            ResultHandler.success("Login processed successfully", HttpStatus.OK.value(), response));
  }
  ...
  @Operation(
      ...
      description =
          "Creates a new account with 'isActive' set to false and "
  ...
  public ResponseEntity<ResultHandler<Void>> register(@RequestBody @Valid RegisterRequest request) {
    ...
    return ResponseEntity.ok(
        ResultHandler.success("User registered successfully", HttpStatus.CREATED.value()));
  }

  @Operation(
      summary = "Activate account with email token (Step 2)",
      description =
          "Finalizes the registration process by activating the user account. "
  ...
  public ResponseEntity<ResultHandler<Boolean>> registerConfirm(@RequestParam String token) {
    ...
    return ResponseEntity.ok(
        ResultHandler.success("User activated successfully", HttpStatus.OK.value(), response));
  }

  @Operation(
      summary = "Request password reset",
      description =
          "Sends a password reset link to the provided email address if the account exists. "
              + "For privacy reasons, always returns a success message.",
      tags = {"Client"})
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Request processed")})
  @PostMapping("/reset-password")
  public ResponseEntity<ResultHandler<Void>> resetPassword(@RequestParam String email) {
    _authServices.resetPassword(email);
    return ResponseEntity.ok(
        ResultHandler.success("If account exists, a link was sent.", HttpStatus.OK.value()));
  }
  ...
  @Operation(
      ...
      description =
          "Accepts a Google ID Token. If the user doesn't exist, "
  ...
  @PostMapping("/google")
  public ResponseEntity<ResultHandler<AuthResponse>> googleAuth(
  ...
  @Operation(
      ...
      description =
          "Validates the 6-digit 2FA code along with the Pre-Auth token. "
  ...
  public ResponseEntity<ResultHandler<AuthResponse>> verify2faLogin(
    ...
    return ResponseEntity.ok()
        .headers(createLoginCookieHeaders(response))
        .body(
            ResultHandler.success("2FA verification successful", HttpStatus.OK.value(), response));
  }
  ...
  public ResponseEntity<ResultHandler<AuthResponse>> refreshToken(
    ...
    return ResponseEntity.ok()
        .headers(createLoginCookieHeaders(response))
        .body(
            ResultHandler.success("Token refreshed successfully", HttpStatus.OK.value(), response));
  }

  @Operation(
      summary = "Get CSRF token",
      description =
          "Protection against browser stupidity. Handle if login doesn't work in production",
      tags = {"Client"})
  @GetMapping("/csrf")
  ...
  private HttpHeaders createLoginCookieHeaders(AuthResponse response) {
    ...
    if (response.getToken() != null) {
      ResponseCookie jwtCookie =
          ResponseCookie.from("accessToken", response.getToken())
              .httpOnly(true)
              .secure(true)
