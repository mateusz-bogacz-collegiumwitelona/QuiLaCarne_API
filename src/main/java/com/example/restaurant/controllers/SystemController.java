...
public class SystemController {
    ...
    public ResponseEntity<ResultHandler<Void>> clearAllCaches() {
        ...
        return ResponseEntity.ok(ResultHandler.success(
                "All caches have been successfully cleared",
                HttpStatus.OK.value()
        ));
    }
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Cache with the given name does not exist"),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
            @ApiResponse(responseCode = "403", description = "Forbidden - requires manager privileges")
    })
    @PreAuthorize("hasAnyRole('ROLE_MANAGER')")
    ...
    public ResponseEntity<ResultHandler<Void>> clearSpecificCache(
        ...
        return ResponseEntity.ok(ResultHandler.success(
                "Cache '" + cacheName + "' cleared successfully",
                HttpStatus.OK.value()
        ));
    }
