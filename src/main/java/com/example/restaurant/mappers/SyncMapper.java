...
public abstract class SyncMapper {
  ...
  public SyncDictionaryResponse toSyncDictionaryResponse(BaseTranslatedEntity entity) {
    ...
    return new SyncDictionaryResponse(entity.getToken(), entity.getNameEn(), entity.getNamePl());
  }

  @Mapping(target = "categoryToken", source = "category", qualifiedByName = "mapCategoryToken")
  @Mapping(
      target = "ingredientTokens",
      source = "ingredients",
      qualifiedByName = "mapIngredientTokens")
  ...
  protected List<String> mapBanStatuses(Set<BanStatus> statuses) {
    return statuses != null ? statuses.stream().map(BaseEntity::getToken).toList() : List.of();
  }
  ...
  public List<String> mapAllergens(Set<Allergens> allergens) {
    return allergens != null ? allergens.stream().map(BaseEntity::getToken).toList() : List.of();
  }
  ...
  @Mapping(
      ...
      qualifiedByName = "mapReservationStatus")
