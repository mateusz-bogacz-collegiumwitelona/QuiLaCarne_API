...
public class OrderServices implements IOrderServices {
  ...
  private void sendOrderSyncEvents(Orders order, List<OrderItems> items, boolean isNew) {
    WebSocketEvent<SyncOrderResponse> orderEvent =
        isNew
            ? WebSocketEvent.created(
                ORDER_ENTITY_TYPE, order.getToken(), _syncMapper.toSyncOrderResponse(order))
    ...
    for (OrderItems item : items) {
      WebSocketEvent<SyncOrderItemResponse> itemEvent =
          isNew
              ? WebSocketEvent.created(
                  "ORDER_ITEM", item.getToken(), _syncMapper.toSyncOrderItemResponse(item))
              : WebSocketEvent.updated(
                  "ORDER_ITEM", item.getToken(), _syncMapper.toSyncOrderItemResponse(item));
