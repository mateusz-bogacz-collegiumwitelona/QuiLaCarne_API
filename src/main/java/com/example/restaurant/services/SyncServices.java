...
public class SyncServices implements ISyncServices {
  ...
  public SyncDictionariesResponse getDictionaries() {
    return SyncDictionariesResponse.builder()
        ...
        .dishCategories(
            _dishRepo.findAllCategories().stream()
                .map(_syncMapper::toSyncDictionaryResponse)
                .toList())
        .banStatuses(
            _banRepo.findAllStatuses().stream().map(_syncMapper::toSyncDictionaryResponse).toList())
        .reportStatuses(
            _reportRepo.findAllStatuses().stream()
                .map(_syncMapper::toSyncDictionaryResponse)
                .toList())
        .orderStatuses(
            _orderRepo.findAllStatuses().stream()
                .map(_syncMapper::toSyncDictionaryResponse)
                .toList())
        .orderItemStatuses(
            _orderRepo.findAllItemStatuses().stream()
                .map(_syncMapper::toSyncDictionaryResponse)
                .toList())
        .reservationStatuses(
            _reservationRepo.findAllStatuses().stream()
                .map(_syncMapper::toSyncDictionaryResponse)
                .toList())
        .tableStatuses(
            _tableRepo.findAllStatuses().stream()
                .map(_syncMapper::toSyncDictionaryResponse)
                .toList())
        .build();
  }
  ...
  private void addModuleMetadata(
      Map<String, SyncBootstrapResponse.EntityMetadata> modules, String key, long count) {
    modules.put(key, new SyncBootstrapResponse.EntityMetadata(count, calculatePage(count)));
  }
