...
public class DataSeeder implements CommandLineRunner {
  ...
  public void run(String... args) {
    ...
    seedNamedEntity(
        _jpaRoleRepo, Roles::new, List.of("ROLE_MANAGER", "ROLE_WAITER", "ROLE_CLIENT"));

    seedTranslatedEntity(
        _jpaTableStatusRepo,
        TableStatus::new,
        List.of(
    ...
    seedTranslatedEntity(
        _jpaGuestReportStatusRepo,
        GuestReportStatus::new,
        List.of(
            new TranslatedDomain("IN_PROGRESS", "W trakcie", "In progress"),
  ...
  private <T extends BaseNamedEntity> void seedNamedEntity(
      JpaRepository<T, ?> repo, Supplier<T> factory, List<String> names) {
    if (repo.count() == 0) {
      names.forEach(
          name -> {
  ...
  private <T extends BaseTranslatedEntity> void seedTranslatedEntity(
      IJpaTranslatedRepository<T> repo, Supplier<T> factory, List<TranslatedDomain> data) {
    if (repo.count() == 0) {
      data.forEach(
          item -> {
            T entity = factory.get();
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "Seeded table: {} ({} items)", factory.get().getClass().getSimpleName(), data.size());
      }
    }
  }
  ...
  private void createMenu() {
    ...
    Ingredients beef =
        createIngredient("Wołowina Chianina", "Chianina Beef", "BEEF_CHIANINA", asSet());
    Ingredients tomato =
        createIngredient("Pomidory San Marzano", "San Marzano Tomatoes", "TOMATOES-SM", asSet());
    Ingredients mozzarella =
        createIngredient(
            "Mozzarella di Bufala", "Buffalo Mozzarella", "MOZZARELLA-BUFFALA", asSet(lactose));
    Ingredients pasta =
        createIngredient(
            "Makaron Tagliatelle", "Tagliatelle Pasta", "TAGLIATELLE-PASTA", asSet(gluten));
    Ingredients parmesan =
        createIngredient("Ser Grana Padano", "Grana Padano Cheese", "GRANA-PADANO", asSet(lactose));
    Ingredients oliveOil = createIngredient("Oliwa z oliwek", "Olive Oil", "OLIVE-OLI", asSet());
    Ingredients bread =
        createIngredient("Pieczywo domowe", "Homemade Bread", "BREAD-HOME", asSet(gluten));
    Ingredients shrimp =
        createIngredient("Krewetki tygrysie", "Tiger Prawns", "SHRIMPS-TIGER", asSet(seafood));
    Ingredients mascarpone =
        createIngredient("Serek Mascarpone", "Mascarpone Cheese", "MASCARPONE", asSet(lactose));
    Ingredients ladyfingers =
        createIngredient("Biszkopty", "Ladyfingers", "LADYFINGERS", asSet(gluten, eggs));
    Ingredients coffee =
        createIngredient("Kawa Espresso", "Espresso Coffee", "COFFEE-ESP", asSet());
    Ingredients rice = createIngredient("Ryż Arborio", "Arborio Rice", "RICE-ARBORIO", asSet());
    Ingredients lemon =
        createIngredient("Cytryny Sycylijskie", "Sicilian Lemons", "LEMON-SICILY", asSet());
    ...
    createDish(
        "Bruschetta Classica", 2500, starterCat, asSet(bread, tomato, oliveOil), "bruschetta.jpg");
    createDish(
        "Carpaccio di Manzo", 4800, starterCat, asSet(beef, parmesan, oliveOil), "carpaccio.jpg");
    createDish("Focaccia Rosmarino", 2200, starterCat, asSet(bread, oliveOil), "focaccia.jpg");

    createDish(
        "Crema di Pomodoro", 2800, soupCat, asSet(tomato, mozzarella, oliveOil), "tomato_soup.jpg");
    ...
    createDish(
        "Tagliatelle Ragu", 4200, mainCat, asSet(pasta, beef, tomato, parmesan), "pasta.jpg");
    ...
    createDish(
        ...
        3500,
        dessertCat,
        asSet(mascarpone, ladyfingers, coffee),
        "tiramisu.jpg");
    createDish("Panna Cotta", 2800, dessertCat, asSet(mascarpone, lemon), "panna_cotta.jpg");
    createDish(
        "Cannoli Siciliani", 3000, dessertCat, asSet(bread, mascarpone, pistachios), "cannoli.jpg");
    ...
    createDish(
        "Salsa al Tartufo", 2000, otherCat, asSet(eggIngredient, oliveOil), "truffle_sauce.jpg");
  }
  ...
  private void createDish(
      String name, int price, DishesCategories cat, Set<Ingredients> ing, String imgName) {
  ...
  private Ingredients createIngredient(
      String pl, String en, String token, Set<Allergens> allergens) {
    ...
    try {
      ...
      if (resource.exists()) {
        return _s3StorageService.uploadFromStream(
            resource.getInputStream(), fileName, "image/jpeg", resource.contentLength());
      } else {
