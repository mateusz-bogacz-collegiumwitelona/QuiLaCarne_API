...
class VerificationTokenServicesTest {
  @Mock private IVerificationTokenRepository _tokenRepo;

  @Mock private IUserRepository _userRepo;

  @InjectMocks private VerificationTokenServices _tokenServices;
  ...
  void createToken_ShouldSaveAndReturnTokenValue() {
    ...
    String tokenValue =
        _tokenServices.createToken(TestConstants.FAKE_USER_TOKEN, TokenTypeEnum.ACTIVATION, 15);
  ...
  void validateToken_WithUserToken_ShouldReturnTrueAndDelete_WhenValid() {
    ...
    boolean result =
        _tokenServices.validateToken("user-token-123", "valid-token", TokenTypeEnum.ACTIVATION);
  ...
  void validateToken_ShouldReturnFalse_WhenTokenExpired() {
    ...
    boolean result =
        _tokenServices.validateToken("user-token-123", "expired-token", TokenTypeEnum.ACTIVATION);
  ...
  void validateToken_ShouldReturnFalse_WhenUserTokenDoesNotMatch() {
    ...
    when(_tokenRepo.findByTokenAndType("token", TokenTypeEnum.ACTIVATION))
        .thenReturn(Optional.of(vt));

    boolean result =
        _tokenServices.validateToken("correct-user", "token", TokenTypeEnum.ACTIVATION);
  ...
  void validateToken_ShouldReturnEmpty_WhenTokenNotFoundInDb() {
    ...
    Optional<String> result =
        _tokenServices.validateToken(TestConstants.TOKEN_NON_EXISTENT, TokenTypeEnum.ACTIVATION);
