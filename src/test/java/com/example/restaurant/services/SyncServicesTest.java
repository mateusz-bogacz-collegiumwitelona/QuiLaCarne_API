...
class SyncServicesTest {
  ...
  @Test
  @DisplayName(
      "Get Dishes Sync: Should correctly map dishes, format S3 URL and extract foreign keys")
  void getDishesSync_ShouldReturnMappedDishes() {
    ...
    assertEquals(
        "https://s3.example.com/my-restaurant/pizzas/margherita.jpg", response.getImageUrl());
  }
  ...
  void getBansSync_ShouldReturnMappedBans() {
    ...
    Page<Bans> mockPage = new PageImpl<>(List.of(ban), PageRequest.of(0, 20), 1);
    when(_banRepo.findAll(any(Pageable.class))).thenReturn(mockPage);

    PagedResult<SyncBanResponse> result = _syncServices.getBansSync(1);
  ...
  void getReportsSync_ShouldReturnMappedReports() {
    ...
    Page<GuestReports> mockPage = new PageImpl<>(List.of(report), PageRequest.of(0, 20), 1);

    when(_reportRepo.findAll(ArgumentMatchers.isNull(), any(Pageable.class))).thenReturn(mockPage);

    PagedResult<SyncReportResponse> result = _syncServices.getReportsSync(1);
  ...
  void getOrdersSync_ShouldReturnMappedOrders() {
    ...
    Page<Orders> mockPage = new PageImpl<>(List.of(order), PageRequest.of(0, 20), 1);
  ...
  void getOrderItemsSync_ShouldReturnMappedItems() {
    ...
    Page<OrderItems> mockPage = new PageImpl<>(List.of(item), PageRequest.of(0, 20), 1);
  ...
  void getReservationsSync_ShouldReturnMappedReservations() {
    ...
    Page<Reservations> mockPage = new PageImpl<>(List.of(reservation), PageRequest.of(0, 20), 1);
    when(_reservationRepo.findAll(ArgumentMatchers.isNull(), any(Pageable.class)))
        .thenReturn(mockPage);
  ...
  void getTablesSync_ShouldReturnMappedTables() {
    ...
    Page<RestaurantTables> mockPage = new PageImpl<>(List.of(table), PageRequest.of(0, 20), 1);
  ...
  @Test
  @DisplayName(
      "Get Users Sync: Should correctly map users, evaluate isStaff flag, and extract role tokens")
  void getUsersSync_ShouldReturnMappedUsers() {
    ...
    Page<Users> mockPage = new PageImpl<>(List.of(user), PageRequest.of(0, 20), 1);
    when(_userRepo.findAllUsers(ArgumentMatchers.isNull(), any(Pageable.class)))
        .thenReturn(mockPage);
  ...
  @Test
  @DisplayName(
      "Get Users Sync: Should set isStaff to false for pure guests and handle missing relations")
