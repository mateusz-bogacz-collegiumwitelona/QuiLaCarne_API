...
class AllergensRepositoryTest {
  @Mock private IJpaAllergensRepository _jpaAllergensRepo;

  @InjectMocks private AllergensRepository _allergensRepo;
  ...
  void findAllergens_ShouldReturnListOfAllergens() {
    ...
    when(_jpaAllergensRepo.findByTokenIn(tokens)).thenReturn(List.of(gluten));
  ...
  void isNameTaken_ShouldReturnTrue_WhenPlNameExists() {
    ...
    boolean result =
        _allergensRepo.isNameTaken(TestConstants.FAKE_ALLERGEN_PL, TestConstants.FAKE_ALLERGEN_EN);
  ...
  void findByToken_ShouldThrowException_WhenNotFound() {
    ...
    com.example.restaurant.exceptions.EntityNotFoundException exception =
        assertThrows(
