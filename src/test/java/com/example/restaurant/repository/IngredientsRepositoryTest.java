...
class IngredientsRepositoryTest {
  ...
  @Mock private IJpaIngredientsRepository _jpaIngredientsRepo;

  @InjectMocks private IngredientsRepository _ingredientsRepository;
  ...
  void isNameTaken_ShouldReturnTrue_WhenPlNameExists() {
    when(_jpaIngredientsRepo.findByNamePl(TestConstants.INGREDIENT_PL))
        .thenReturn(Optional.of(new Ingredients()));

    boolean result =
        _ingredientsRepository.isNameTaken(
            TestConstants.INGREDIENT_PL, TestConstants.INGREDIENT_EN);
  ...
  void isNameTaken_ShouldReturnTrue_WhenEnNameExists() {
    ...
    when(_jpaIngredientsRepo.findByNameEn(TestConstants.INGREDIENT_EN))
        .thenReturn(Optional.of(new Ingredients()));
  ...
  void findByToken_ShouldReturnIngredient_WhenExists() {
    Ingredients ingredient = new Ingredients();
    when(_jpaIngredientsRepo.findByToken(TestConstants.TOKEN_TOMATO))
  ...
  void findByToken_ShouldThrowException_WhenNotFound() {
    when(_jpaIngredientsRepo.findByToken(TestConstants.TOKEN_NON_EXISTENT))
        .thenReturn(Optional.empty());

    RuntimeException exception =
        assertThrows(
            RuntimeException.class,
            () -> _ingredientsRepository.findByToken(TestConstants.TOKEN_NON_EXISTENT));
