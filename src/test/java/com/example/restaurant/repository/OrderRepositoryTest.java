...
class OrderRepositoryTest {
  ...
  @Mock private IJpaOrderItemStatusRepository _jpaOrderItemStatusRepo;

  @InjectMocks private OrderRepository _orderRepo;
  ...
  void findStatusByToken_ShouldReturnStatus_WhenFound() {
    ...
    when(_jpaOrderStatusRepo.findByToken("PENDING")).thenReturn(Optional.of(mockStatus));
  ...
  void findStatusByToken_ShouldThrowException_WhenNotFound() {
    when(_jpaOrderStatusRepo.findByToken(TestConstants.TOKEN_NON_EXISTENT))
        .thenReturn(Optional.empty());

    assertThrows(
        RuntimeException.class,
        () -> _orderRepo.findStatusByToken(TestConstants.TOKEN_NON_EXISTENT));
  }
  ...
  void findItemStatusByToken_ShouldThrowException_WhenNotFound() {
    when(_jpaOrderItemStatusRepo.findByToken(TestConstants.TOKEN_NON_EXISTENT))
        .thenReturn(Optional.empty());

    assertThrows(
        RuntimeException.class,
        () -> _orderRepo.findItemStatusByToken(TestConstants.TOKEN_NON_EXISTENT));
  }
  ...
  void findAllItemStatuses_ShouldReturnListOfItemStatuses() {
    List<OrderItemsStatus> expectedStatuses =
        List.of(new OrderItemsStatus(), new OrderItemsStatus());
  ...
  void isItemStatusNameTaken_ShouldReturnTrue_WhenPlNameExists() {
    when(_jpaOrderItemStatusRepo.findByNamePl(anyString()))
        .thenReturn(Optional.of(new OrderItemsStatus()));
  ...
  void isItemStatusNameTaken_ShouldReturnTrue_WhenEnNameExists() {
    ...
    when(_jpaOrderItemStatusRepo.findByNameEn(anyString()))
        .thenReturn(Optional.of(new OrderItemsStatus()));
