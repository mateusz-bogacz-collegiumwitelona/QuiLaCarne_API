...
class RoleRepositoryTest {
  ...
  @InjectMocks private RoleRepository _roleRepository;

  @Test
  @DisplayName("Set role: should throw exception if role not found")
  void setRole_ShouldThrowException_WhenRoleNotFound() {
    when(_jpaRoleRepository.findByName(TestConstants.TOKEN_NON_EXISTENT))
        .thenReturn(Optional.empty());

    assertThrows(
        RuntimeException.class, () -> _roleRepository.setRole(TestConstants.TOKEN_NON_EXISTENT));
  }
  ...
  void setRole_ShouldReturnRole_WhenFound() {
    ...
    when(_jpaRoleRepository.findByName(TestConstants.ROLE_CLIENT))
