...
class UserRepositoryTest {
  @InjectMocks private UserRepository _userRepository;

  @Mock private IJpaUserRepository _jpaUserRepository;

  @Mock private IJpaRoleRepository _jpaRoleRepository;

  @Test
  @DisplayName("exists By Username: should return true when user exists")
  void existsByUsername_ShouldReturnTrue_WhenUserExists() {
    when(_jpaUserRepository.findByNormalizedUsername(anyString()))
  ...
  void isInRole_ShouldReturnTrue_WhenUserHasThisRole() {
    ...
    when(_jpaUserRepository.findByToken(TestConstants.FAKE_USER_TOKEN))
        .thenReturn(Optional.of(mockUser));
    when(_jpaRoleRepository.findByName(TestConstants.FAKE_ROLE))
  ...
  void findByNormalizedEmail_ShouldCallJpa() {
    when(_jpaUserRepository.findByNormalizedEmail(anyString()))
        .thenReturn(Optional.of(new Users()));
  ...
  void isInRole_ShouldReturnFalse_WhenUserDoesNotHaveTheRole() {
    ...
    when(_jpaRoleRepository.findByName(TestConstants.ROLE_MANAGER))
        .thenReturn(Optional.of(searchRole));

    assertFalse(
        _userRepository.isInRole(TestConstants.ROLE_MANAGER, TestConstants.FAKE_USER_TOKEN));
  }
