...
class EmailQueueConsumerTest {
  @Mock private StringRedisTemplate _redisTemplate;

  @Mock private ListOperations<String, String> _listOperations;

  @Mock private ObjectMapper _mapper;

  @Mock private JavaMailSender _sender;

  @Mock private TemplateEngine _templateEngine;

  @Mock private MimeMessage _mimeMessage;

  @InjectMocks private EmailQueueConsumer _emailQueueConsumer;
  ...
  void shouldProcessEmailSuccessfully() throws Exception {
    String validJson =
        "{\"to\":\"test@test.com\",\"subject\":\"Test\",\"template\":\"test-template\"}";
    EmailDomain emailDomain =
        new EmailDomain("test@test.com", "Test Subject", "test-template", Map.of("key", "value"));
    ...
    when(_templateEngine.process(
            eq(emailDomain.template()), any(org.thymeleaf.context.Context.class)))
  ...
  void shouldHandleExceptionWhenJsonIsInvalid() throws JsonProcessingException {
    ...
    when(_mapper.readValue(invalidJson, EmailDomain.class))
